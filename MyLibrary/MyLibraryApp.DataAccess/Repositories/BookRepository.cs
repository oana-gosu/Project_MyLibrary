...
namespace MyLibraryApp.DataAccess.Repositories
{
    public class BookRepository : IBookRepository
    {
        ...
        {
            return await databaseContext.Books.Where(b => b.Author.Contains(author)).ToListAsync();
        }

        public async Task<List<Book>> GetAllByCategory(int category)
        {
            return await databaseContext.Books.Where(b => b.CategoryId == category).ToListAsync();
        }

        public async Task<List<Book>> GetAllByTitle(string title)
        {
            return await databaseContext.Books.Where(b => b.Title.Contains(title)).ToListAsync();
        }
